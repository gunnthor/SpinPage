...
- **The Gremlin**: as the wheel crawls through its last degree, something climbs onto the rim and takes hold of it. It creeps the wheel round by hand — a shove, then a dead-still hold parked on somebody's name, then another shove — and every so often it gives the wheel one last nudge, takes its hands off and looks innocently away while the wheel settles. It has landed. It is over. Then it grabs the wheel again. That repeats up to five times, so a gremlin is anywhere from twelve seconds to the better part of a minute, and it gets likelier the emptier the wheel gets: 5% at a full table, 30% with three names left.
